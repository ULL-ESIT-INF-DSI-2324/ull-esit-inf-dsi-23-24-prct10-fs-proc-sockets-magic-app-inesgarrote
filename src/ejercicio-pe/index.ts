    ...
    handler: (argv) => {
      ...
      if (argv.tipo === Tipo.Criatura) {
        if (argv.fuerzaResistencia) {
          try {
            fuerzaResistenciaArray = JSON.parse(argv.fuerzaResistencia) as [
              number,
              number,
            ];
          } catch (error) {
            console.error(
              chalk.red("Error al analizar fuerzaResistencia:"),
              error,
            );
          ...
          console.log(
            chalk.red(
              "El atributo fuerzaResistencia es requerido para cartas de tipo Criatura.",
            ),
          );
          process.exit(1); // Salir del proceso con un código de error
        }
...
yargs(hideBin(process.argv))
  .command({
    ...
    handler: (argv) => {
      ...
      usuario.agregarCarta(carta, (error) => {
        if (error) {
          console.error(chalk.red("Error al agregar la carta:"), error);
          process.exit(1);
        }
      });
    },
  ...
  .command({
    ...
    handler: (argv) => {
      ...
      usuario.listarCartas((error) => {
        if (error) {
          console.error(chalk.red("Error al listar las cartas:"), error);
          process.exit(1);
        }
      });
    },
  ...
  .command({
    ...
    handler: (argv) => {
      ...
      usuario.mostrarCarta(argv.id as number, (error) => {
        if (error) {
          console.error(chalk.red("Error al mostrar la carta:"), error);
          process.exit(1);
        }
      });
    },
  ...
  .command({
    ...
    handler: (argv) => {
      ...
      // Modificar la carta
      usuario.modificarCarta(
        argv.id as number,
        cartanueva as Carta,
        (error) => {
          if (error) {
            console.error(chalk.red("Error al modificar la carta:"), error);
            process.exit(1);
          }
        },
      );
  ...
  .command({
    ...
    handler: (argv) => {
      ...
      usuario.eliminarCarta(argv.id as number, (error) => {
        if (error) {
          console.error(chalk.red("Error al eliminar la carta:"), error);
          process.exit(1);
        }
      });
    },
